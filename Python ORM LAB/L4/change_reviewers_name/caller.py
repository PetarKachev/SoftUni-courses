...
def find_authors_nationalities():
    ...
    for author in authors:
        author_names.append(f"{author.first_name} {author.last_name} is {author.nationality}")
...
def order_books_by_year():
    ...
    for book in books:
        book_names.append(f"{book.publication_year} year: {book.title} by {book.author}")
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in authors:
        if author.biography:
            author_names.append(f"{author.biography}")
        else:
...
def filter_authors_by_birth_year(first_year, second_year):
    authors = Author.objects.filter(birth_date__year__gte=first_year, birth_date__year__lte=second_year).order_by(
        '-birth_date')
    author_names = []

    for author in authors:
        author_names.append(f"{author.birth_date}: {author.first_name} {author.last_name}")
